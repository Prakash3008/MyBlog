...
from flask_blog.users.forms import RegistrationForm, LoginForm, UpdateForm, RequestReset, ResetPassword
...
def reset_request():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(email=form.email.data).first()
        send_email(user)
        flash('A password reset link has been sent your mail ID','info')
